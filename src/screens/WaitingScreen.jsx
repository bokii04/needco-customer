...
export default function WaitingScreen() {
  const { navigate, activeJob, setActiveJob, setSelectedWorker, user } = useApp();
  ...
  useEffect(() => {
    ...
    channelRef.current = supabase
      ...
      .on("postgres_changes", {
        ...
        console.log("[Customer] Job updated:", updated.status);
        setJob(updated);
        setActiveJob(updated);

        if (updated.status === "accepted" || updated.status === "active") {
          if (updated.worker_id) {
            const { data: w } = await supabase.from("workers").select("*").eq("user_id", updated.worker_id).maybeSingle();
            if (w) {
              setWorkerInfo(w);
              setSelectedWorker({
                id: w.id,
                name: updated.worker_name || w.full_name,
                initials: (updated.worker_name || w.full_name || "WK").split(" ").map(n => n[0]).join("").substring(0, 2).toUpperCase(),
                rating: w.rating || 4.8,
                jobs: w.total_jobs || 0,
                distance: 1.2,
                skills: w.skills || [],
                price: { [updated.service]: updated.price },
              });
            }
            setWorkerFound(true);
        ...
        if (updated.status === "cancelled") navigate("home");
      })
      .subscribe();
        ...
        {workerInfo && (
          <div className="fade-up-3 card" style={{ marginTop: 20, width: "100%", display: "flex", alignItems: "center", gap: 14 }}>
            <div className="avatar" style={{ width: 52, height: 52, background: "var(--gold)", color: "var(--black)", fontSize: 18, fontWeight: 700 }}>
              {(workerInfo.full_name || "WK").split(" ").map(n => n[0]).join("").substring(0, 2).toUpperCase()}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: 16 }}>{workerInfo.full_name}</div>
              <div style={{ fontSize: 13, color: "var(--gray-400)", marginTop: 2 }}>⭐ {workerInfo.rating || "New"} · {workerInfo.total_jobs || 0} jobs · {workerInfo.city}</div>
            </div>
            <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: 20, color: "var(--gold-dark)" }}>₱{job?.price}</div>
          </div>
        )}
        <p className="fade-up-4" style={{ color: "var(--gray-400)", fontSize: 13, marginTop: 16, textAlign: "center" }}>Redirecting to live tracking...</p>
      </div>
    );
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "32px 24px", textAlign: "center" }}>
        ...
        <p style={{ color: "var(--gray-400)", fontSize: 14, lineHeight: 1.7, marginBottom: 8 }}>
          Nearby workers are being notified of your {job?.service} request.<br />You'll be matched as soon as someone accepts.
        </p>
        ...
        <div className="card" style={{ width: "100%", marginBottom: 20, textAlign: "left" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
            ...
            <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: 22, color: "var(--black)" }}>₱{job?.price}</div>
          </div>
